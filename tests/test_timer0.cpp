...
TEST_CASE("Interrupt: raise and service dispatches to correct vector", "[interrupt]") {
    ...
    // PC should now point to TIMER0_OVF vector: (17 - 1) * 2 = 32 = 0x0020
    REQUIRE(s.pc == 0x0020);
...
TEST_CASE("Interrupt: highest priority wins", "[interrupt]") {
    ...
    bool dispatched = interruptService();
    REQUIRE(dispatched);
    // TIMER0_COMPA at (15-1)*2 = 28 = 0x001C
    REQUIRE(s.pc == 0x001C);
    ...
    dispatched = interruptService();
    REQUIRE(dispatched);
    REQUIRE(s.pc == 0x0020);   // TIMER0_OVF
}
...
TEST_CASE("E2E: Timer0 overflow raises interrupt, ISR runs", "[timer0][interrupt][e2e]") {
    ...
    // Set up interrupt controller: I=1, put a RETI at vector 0x0020 in flash
    setFlag(s, SregBit::I);
    s.pc = 0x0100;

    // Place a RETI instruction (0x9518) at the TIMER0_OVF vector address
    // TIMER0_OVF = vector 17 → byte address 0x0020
    s.flash[0x0020] = 0x18;
    s.flash[0x0021] = 0x95;  // RETI = 0x9518 (little-endian: low byte 0x18, high byte 0x95)
    ...
    // Service the interrupt — should jump to ISR at 0x0020
    bool dispatched = interruptService();
    REQUIRE(dispatched);
    REQUIRE(s.pc == 0x0020);
    timer0AckOverflow();
